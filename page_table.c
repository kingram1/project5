...
static void internal_fault_handler(int signum, siginfo_t * info,
				   void *context)
{
				     ...
				     page_fault_handler_t handler)
{
    ...
    pt->physmem =
	mmap(0, nframes * PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED,
	     pt->fd, 0);
    pt->nframes = nframes;

    pt->virtmem =
	mmap(0, npages * PAGE_SIZE, PROT_NONE, MAP_SHARED | MAP_NORESERVE,
	     pt->fd, 0);
    pt->npages = npages;
			  ...
			  int bits)
{
    ...
    remap_file_pages(pt->virtmem + page * PAGE_SIZE, PAGE_SIZE, 0, frame,
		     0);
